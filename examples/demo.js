...
console.log( 'Totemheart — emotional coherence simulation (computed, inspectable state).' )
console.log( 'This is NOT real feeling: there is no subjective experience anywhere in this process.' )
console.log( 'Sources and limitations for each mechanism: see CALIBRATION.md.\n' )
...
for ( const [ i, input ] of turns.entries() ) {
	...
	if ( result.emotionalState ) {
		...
		console.log( 'dominant:', result.emotionalState.dominantEmotion, '| cognitive stress:', result.emotionalState.cognitiveStress.toFixed( 2 ) )
		console.log( '| cortisol:', result.emotionalState.cortisol.toFixed( 2 ), '| egoHealth:', result.emotionalState.egoHealth.toFixed( 2 ) )

	}
	if ( result.debug ) console.log( 'RPE (dopaminergic surprise):', result.debug.rpe.toFixed( 2 ) )

	totemheart.tick( 3 )

}

console.log( '\n--- Circadian rhythm right now ---' )
console.log( totemheart.circadianRhythm.getState() )

console.log( '\n--- Simulating high API latency (hardware interoception) ---' )
...
console.log( '\n--- Simulating a message burst (sensory overload) ---' )
...
console.log( '\n--- Simulating a group chat (bystander effect) ---' )
const group = await totemheart.processInput( 'alguien sabe qué hora es?', {
	userId    : 'demo-user',
	group     : { participantCount: 8, mentionedExplicitly: false },
} )
console.log( 'Did it respond?', group.respond !== false )

console.log( '\n--- Generic system prompt to inject into any LLM (Claude/GPT/Ollama/...) ---' )
console.log( totemheart.getSystemPrompt( { userId: 'demo-user' } ) )

console.log( '\n--- Idle cycle (rumination) ---' )
await totemheart.idle()
console.log( 'State after idle():', totemheart.getEmotionalState() )

console.log( '\n--- Explainability report (last 3 decisions) ---' )
