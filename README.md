...
**Revised down in 1.6.0, on purpose, not up.** From 0.1.3 through round 5 of this release the headline stayed flat because new mechanisms were individually cited but too few in number to move a system-wide estimate. That stopped being true this round: 1.6.0 added roughly 40 new mechanisms, and per the per-mechanism table below, most of the newest ones (Friki Engine, the somatic/mood/ToT suite, the discourse-shaper/blush/PoA suite, relational memory catalog) score in the **~28-52%** range, well *below* the ~80-95% the State engine / Expression / Cross-turn continuity rows were sitting at. Diluting a row average with several own-design-heavy mechanisms pulls that row's real grounding down, not up — the same logic the README already stated ("several score BELOW the layer averages they landed in") now has enough weight behind it to actually move the numbers instead of just being a disclaimer. So: State engine, Expression/output, and Cross-turn continuity all moved down a few points from where they were pre-1.6.0; Semantic understanding is essentially unchanged (its 1.6.0 additions were smaller in number and mid-range in score, not enough to shift it). Full citation ledger: [`CALIBRATION.md`](CALIBRATION.md).
